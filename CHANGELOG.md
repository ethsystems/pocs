...
- **Added**: a Noir fixture for the ungated deposit statement of 2/SHIELDED-POOL, in `tests/core_deposit/`. Eight tests cover the commitment relation, the u128 amount bound, and five rejection cases, against a free owner public key. The circuit joins the Nargo workspace, so `nargo test --workspace` runs it. `cargo test --test core_deposit_fixture` proves both deposit statements with Barretenberg FFI and checks that each of the five core public inputs is bound, and that neither deposit mode verifies under the other's key. The attested PoC is unchanged, and the fixture does not establish core-only conformance.
